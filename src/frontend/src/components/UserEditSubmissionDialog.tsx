...
import { useUpdateSongSubmission } from '../hooks/useQueries';
import { ExternalBlob, type SongSubmission } from '../backend';
...
export default function UserEditSubmissionDialog({ submission, open, onOpenChange }: UserEditSubmissionDialogProps) {
  ...
  const updateSubmission = useUpdateSongSubmission();
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <div className="border-2 border-dashed rounded-lg p-4 text-center">
                <img src={artworkPreview} alt="Artwork" className="w-24 h-24 mx-auto rounded-lg object-cover mb-2" />
                <Button type="button" variant="outline" size="sm" onClick={() => document.getElementById('artwork-edit')?.click()}>
            ...
            <div className="space-y-2">
              ...
              <div className="border-2 border-dashed rounded-lg p-4 text-center">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => document.getElementById('audio-edit')?.click()}>
                  <Upload className="w-4 h-4 mr-2" />
                  Change Audio
                </Button>
                {audioFile && <p className="text-xs mt-2">{audioFile.name}</p>}
                <input
                  id="audio-edit"
                  type="file"
                  accept="audio/*"
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button type="submit" disabled={updateSubmission.isPending || isUploading}>
              {updateSubmission.isPending || isUploading ? 'Saving...' : 'Save Changes'}
            </Button>
          </div>
        </form>
      </DialogContent>
