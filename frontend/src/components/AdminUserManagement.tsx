...
import { UserCategory, ArtistProfile } from '../backend';
...
export default function AdminUserManagement() {
  ...
  const uniqueUsers: ArtistProfile[] = artistProfiles
    ? Array.from(
        new Map(
          artistProfiles.map((profile: ArtistProfile) => [profile.owner.toString(), profile])
        ).values()
      )
    : [];
    ...
    const loadUserData = async () => {
      ...
      for (const profile of uniqueUsers) {
        try {
          ...
          newStatuses.set(profile.owner.toString(), { songBlocked: false, podcastBlocked: false });
        }
  ...
  const handleToggleSongBlock = async (user: Principal) => {
    ...
    try {
      ...
      const msg = error instanceof Error ? error.message : 'Failed to update block status';
      toast.error(msg);
    }
  };
                        ...
                        <TableCell className="text-center">
                          <div className="flex items-center justify-center gap-2">
                            <Switch
                              ...
                              disabled={
                                blockPodcastMutation.isPending || unblockPodcastMutation.isPending
                              }
                            />
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Switch
                        ...
                        onCheckedChange={() => handleToggleSongBlock(selectedUser)}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Switch
                        checked={
                          blockStatuses.get(selectedUser.toString())?.podcastBlocked || false
                        }
                        onCheckedChange={() => handleTogglePodcastBlock(selectedUser)}
                        disabled={
                          blockPodcastMutation.isPending || unblockPodcastMutation.isPending
                        }
                      />
                      <span className="text-sm text-muted-foreground">
                        {blockStatuses.get(selectedUser.toString())?.podcastBlocked
                          ? 'Blocked'
                          : 'Active'}
                      </span>
                    </div>
