        ...
        {!sortedVideos || sortedVideos.length === 0 ? (
          <Card>
            <CardContent className="py-12">
              <p className="text-center text-muted-foreground">No video submissions found.</p>
            </CardContent>
          </Card>
        ) : (
              ...
              <Card key={video.id}>
                  ...
                  <div className="flex justify-between items-start flex-wrap gap-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {new Date(Number(video.submittedAt / BigInt(1_000_000))).toLocaleDateString()}
                      </p>
                    </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                    ...
                    {video.tags && video.tags.length > 0 && (
                      <div className="col-span-2">
                        ...
                        <span className="font-medium">{video.tags.join(', ')}</span>
                      </div>
                    )}
                    {video.liveUrl && (
                  ...
                  <div className="space-y-2">
                    <Label className="text-xs">Live URL (required when setting to Live)</Label>
                    <Input
                      placeholder="https://..."
                      value={liveUrls[video.id] || ''}
                      onChange={(e) =>
                        setLiveUrls((prev) => ({ ...prev, [video.id]: e.target.value }))
                      }
                      className="text-sm"
                    />
                  </div>
