...
import { Plus, Trash2, Loader2, Music, User, Image, Save, AlertCircle } from 'lucide-react';
import { useGetFeaturedArtists, useSetFeaturedArtist, useToggleFeaturedArtistSlot } from '../hooks/useQueries';
...
function SlotCard({
  ...
  const addSong = () => {
    if (form.songs.length < 3) {
      setForm((prev) => ({ ...prev, songs: [...prev.songs, { title: '', link: '' }] }));
    }
  };
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {isToggling && <Loader2 className="w-4 h-4 animate-spin text-amber-400" />}
            <Badge
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Textarea
            ...
            onChange={(e) => setForm((prev) => ({ ...prev, aboutBlurb: e.target.value }))}
...
export default function AdminFeaturedArtistsManagement() {
  ...
  const getSlotData = (slotIndex: number): FeaturedArtist | null => {
    ...
    return featuredArtists.find((a) => Number(a.slotIndex) === slotIndex) ?? null;
  };

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-foreground mb-1">Most Vibing Artists</h2>
        <p className="text-muted-foreground text-sm">
          Manage up to 3 featured artist slots displayed on the public landing page carousel. Toggle
          each slot ON to make it visible to visitors.
        </p>
      </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-16">
          ...
          <span className="ml-3 text-muted-foreground">Loading featured artists...</span>
        </div>
      ) : (
