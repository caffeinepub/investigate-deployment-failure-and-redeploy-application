...
import { useListAdmins, usePromoteToAdmin, useDemoteAdmin } from '../hooks/useQueries';
...
import { ShieldCheck, UserMinus, UserPlus, Copy, Check, AlertCircle, Loader2 } from 'lucide-react';
...
export default function AdminUserRoleManagement() {
  ...
  const handlePromote = async () => {
    ...
    try {
      ...
      setPromoteSuccess(`Successfully promoted ${truncatePrincipal(trimmed)} to admin.`);
      setPromoteInput('');
    } catch (err: unknown) {
  ...
  const handleDemote = async (principalStr: string) => {
    ...
    try {
      ...
      setDemoteErrors((prev) => ({ ...prev, [principalStr]: 'Invalid principal.' }));
      return;
    }
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              All principals with admin privileges. The last remaining admin cannot be demoted.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              ...
              <div className="space-y-3">
                {admins.map((principal) => {
                  ...
                  const isDemoting =
                    demoteAdmin.isPending && demoteAdmin.variables?.toString() === principalStr;
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="font-mono text-sm truncate" title={principalStr}>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        {demoteError && (
                          <span className="text-xs text-destructive max-w-[160px] truncate" title={demoteError}>
                            {demoteError}
                          </span>
                        )}
